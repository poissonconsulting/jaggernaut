...
#'
#' @description
#' Get the select_derived component of a JAGS object.  
#' 
#' @param object a JAGS object.
#' @param ... further arguments passed to or from other methods.
#' @return The select_derived component of a JAGS object.
#' @seealso \code{\link{jaggernaut}}  
#' @export
select_derived <- function (object, ...) {
...
#'
#' @description
#' Set the select_derived component of a JAGS object.  
#' 
#' @usage
#' select_derived(object) <- value
#' @param object a JAGS object.
#' @param value a function or NULL to replace the modify data derived fnction.
#' @seealso \code{\link{jaggernaut}}  
#' @export
"select_derived<-" <- function (object, value) {
  UseMethod("select_derived<-", object)
}
...
#' @method select_derived jags_model
#' @export
select_derived.jags_model <- function (object, ...) {
...
#' @method select_derived jags_analysis
#' @export
select_derived.jags_analysis <- function (object, ...) {
...
#' @method select_derived<- jags_model
#' @export
"select_derived<-.jags_model" <- function (object, value) {
  ...
  for (i in 1:length(models)) {
    if(!is.list(value)) {
      select_derived(models[[i]]) <- value
    } else
      select_derived(models[[i]]) <- value[[i]]
  }
...
#' @method select_derived<- jags_analysis
#' @export
"select_derived<-.jags_analysis" <- function (object, value) {
  
  if (!is.null (value)) {
    if(!is.character(value)) {
      stop("select_derived must be NULL or a character")
    }
  }
  ...
  for (i in 1:nmodels(object))
    select_derived(object$analyses[[i]]) <- value
